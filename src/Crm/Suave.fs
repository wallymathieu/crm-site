...
module Successful=
  ...
  let CREATED s= OptionT<< (S.CREATED s)
...
module Json=
  ...
  open RequestErrors
  open Writers
  let withMimeType webPart = webPart >=> setMimeType "application/json; charset=utf-8"

  let inline OK (v:JsonValue) : WebPart= withMimeType <| OK (string v)
  let inline CREATED (v:JsonValue) : WebPart= withMimeType <| CREATED (string v)

  let inline BAD_REQUEST (v:JsonValue) : WebPart=withMimeType <| BAD_REQUEST (string v)

  let inline getBody (ctx : Suave.Http.HttpContext)=
    ctx.request.rawForm |> System.Text.Encoding.UTF8.GetString |> JsonValue.TryParse
